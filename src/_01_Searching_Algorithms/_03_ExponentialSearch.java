...
public class _03_ExponentialSearch {
	...
	// Exponential search is super efficient for large data sets by optimizing the binary search. It, like the binary search, only works with sorted data.
	
	//1. Complete the steps in the exponentialSearch method. We can assume that array is sorted.
	public static int exponentialSearch(int array[], int value)
	{
		// 2. Check if the array element at 0 is the value. If it is, then return 0.
if (array[0] == value) {
	return 0;
}
	...
	minimum = array.length - 1;
		...
		//9. use the binary search method you wrote using the following parameters:	the array to be searched half of the counter minimum the value to be found
		int hold = _01_BinarySearch.binarySearch(array, counter / 2, minimum, value);
