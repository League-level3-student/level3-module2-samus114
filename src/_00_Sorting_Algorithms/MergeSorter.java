...
public class MergeSorter extends Sorter {
	...
	// 0. Since this sorting algorithm uses recursion, we'll make a helper method
	// called mergeSort. Complete the steps in the mergeSort method. You can use
	...
	private void mergeSort(int[] array, int low, int high, SortingVisualizer display) {
		...
		// 2. make an if statement that checks if low is less than high and put the rest
		// of the method inside of it
		if (low < high) {
			...
			// 6. copy the elements from the array into the temporary array, but only the
			// elements from low to high inclusive
			...
			while (i <= middle && j <= high) {
				// 9. if temp array at i is less than or equal to temp array at j
				if (tempA[i] <= tempA[j]) {
