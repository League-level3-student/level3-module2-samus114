...
public class HeapSorter extends Sorter {
	...
	// called heapSort. Complete the steps in the heapSort method. You can use
	// display.updateDisplay() to show the current progress on the graph.
	@Override
	void sort(int[] array, SortingVisualizer display) {
		// 8. create an integer called n and set it equal to the length of the array
		int n = array.length;
		// 9. make a for loop that starts half way between 0 and n and counts down until
		// it is less than 0. Inside this for loop, call the heapSort method with n and
		// i
		for (int i = n / 2; i >= 0; i--) {
			heapSort(array, n, i, display);
			display.updateDisplay();
		}
		// 10. make a for loop that starts at n-1 and counts down until it is less than
		// 0.
		for (int i = n - 1; i >= 0; i--) {
			int temp = array[0];
